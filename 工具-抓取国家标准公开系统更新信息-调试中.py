...
url = 'https://openstd.samr.gov.cn/bzgk/gb/notice?r=0.136134950662663&page=1&pageSize=10'
res = urllib.request.urlopen(url)
html = res.read().decode('utf-8')
soup = BeautifulSoup(html, 'lxml')
result = soup.find_all('div', class_='list-group')
# print(result)
...
for a_url in url_all:
    ...
    a_url = a_url.replace('javascript:searchN(','')
    a_url = a_url.replace(')','')
    a_url = a_url.replace("'","")
    # print('a_url--------------------',a_url)
...
#-----------------------------文件夹及文件的生成及读写----------------------------------------------------
# 定义txt存储路径。
picpath = './GB/'  # 这里我用的是本程序路径，也可改为c盘或d盘等路径。
...
for i in range(1):
    try:
        res = urllib.request.urlopen('https://openstd.samr.gov.cn/bzgk/gb/nd?no='+urls[i]) #把变量urls中的链接地址补齐，由变量res获取链接中的内容
        html = res.read().decode('utf-8') #编码
        soup = BeautifulSoup(html, 'lxml') #lxml处理
        soup = soup.find('div',class_ = 'content')
        # print(str(i+1) + 'saved')

        for p in soup.select('tbody'):
            t = p.get_text()
            print (t)
            txt(titles[i], t)
